...
module.exports = class Chatroom {
  ...
  processTasks() {
    if(this.taskBuffer.length) {
      ...
      console.log('Should be done eventually..', this.taskBuffer[0].done)
      if(this.taskBuffer[0].done) {
        console.log('inside:',this.taskBuffer[0].done)
        ...
        for(let id in this.clients) {
          ...
          if(this.clientSockets.hasOwnProperty(id)) {
            ...
            clientSocket.send(this.taskBuffer[0].translatorHash[this.clients[id].language].synthesisResult);
          }
        }
