...
.transcribe = async function(transcriptionRequest) {
...
Transcriber.prototype
.makeTranscriptionRequest=function(fileName, inLanguage) {
...
Transcriber.prototype.speech2Text = async function() {
  ...
  this.transcriptionRequest = this.makeTranscriptionRequest(this.fileName, this.language);
  this.transcriptionResult = await this.transcribe(this.transcriptionRequest);
...
Translator.prototype.synthesize = async function() {
  // Performs the Text-to-Speech request
  let results = await this.synthesisClient.synthesizeSpeech(this.synthesisRequest)
...
Translator.prototype.text2TransSpeech = async function(){
  ...
  this.synthesisRequest = this.makeSynthesisRequest(this.translationResult, this.outLang)
  this.synthesisResult = await this.synthesize(this.synthesisRequest);
  console.log('Request:', this.synthesisRequest)
...
class Task{
  constructor(inAudio, inLang, outLang){
    this.transcriber = new Transcriber(inAudio, inLang)
    this.translator
  }
}
...
// let testTranscriber =  new Transcriber(fileName, language)
// testTranscriber.speech2Text().then(text => )
// // console.log(text)
// console.log(await testTranscriber.speech2Text())

let testTranslator = new Translator("I went to the park today",'en-US',"ko-KR")
// let audio = await testTranslator.text2TransSpeech()

testTranslator.text2TransSpeech().then(data=>console.log(data))
